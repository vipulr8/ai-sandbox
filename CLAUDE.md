...
VS Code extensions are also configured in two places that drift apart: `dev.sh:223-231` has a hardcoded install loop (including `anthropic.claude-code@<version>`), while `.devcontainer/devcontainer.json` has a separate declarative `extensions` array (no Claude entry). Adding an extension means updating both.
